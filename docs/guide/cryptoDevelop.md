...
目前ddn为用户提供两种加密算法，一种是基于napm包[tweetnacl](https://www.npmjs.com/package/tweetnacl)实现,一种是基于[ed25519](https://www.npmjs.com/package/ed25519)实现，如果这两种加密算法不能满足用户需求，用户可以定制开发自己需要的加密算法，只要提供ddn需要的相关api，就能无缝衔接

DDN采用的是模块化开发模式，使用learn来管理项目，当然加密算法也是其中的一个包，所以用户只需按照要求开发需要的加密算法包，然后安装，在配置文件里配置就可一在DDN网络里使用，下面是DDN加密算法开发和加载逻辑
...
该方法是获取给定字节的数据（一般是通过3.1返回创建的数据），该方法返回的数据类型必须和3.4返回的数据类型一致

参数说明

|名称	|类型   |必填 |说明 |默认|
|------ |-----  |---  |----|---              |
|data|通过3.1获取的数据|Y|需要获取hash的数据|
