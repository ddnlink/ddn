...
class Issue extends Asset.Base {
  ...
  async calculateFee() {
    return bignum.multiply(
      this.constants.net.fees.aob_issue,
  ...
  async verify(trs, sender) {
    ...
    for (let i = 0; i < result4.length; i += 1) {
      ...
      result4[i].maximumShow = DdnJS.amount.calcRealAmount(
        result4[i].maximum,
        precision
      );
      result4[i].quantity = bignum.new(result4[i].quantity).toString(10);
      result4[i].quantityShow = DdnJS.amount.calcRealAmount(
        result4[i].quantity,
        precision
      );
    }
    ...
    if (result5.issuer_id !== sender.address) {
      throw new Error(
        "Permission not allowed --- from asset-aob -> issue.verify"
      );
    }
    if (result5.writeoff) {
      throw new Error(
        "Asset already writeoff --- from asset-aob -> issue.verify"
      );
    }
    ...
    if (strategy) {
      ...
      if (!evalRet) {
        throw new Error(
          "Strategy not allowed --- from asset-aob -> issue.verify"
        );
      }
    }
    return null;
  ...
  async undo(trs, block, sender, dbTrans) {
    ...
    const { currency, amount } = assetIssue;
    const balance =
      this.balanceCache.getAssetBalance(sender.address, currency) || 0;
