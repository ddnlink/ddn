...
class Transfer extends Asset.Base {
  ...
  async calculateFee() {
    return DdnUtils.bignum.multiply(
      this.constants.net.fees.aob_transfer,
  ...
  async verify(trs, sender) {
    ...
    if (assetDetail.acl === 0) {
      ...
      if (
        (sender.address !== issuerInfo.issuer_id &&
  ...
  async apply(trs, block, sender, dbTrans) {
    ...
    const assetBalancedata = await this.dao.findOne(
      ...
      ["balance"]);
    const balance =
      assetBalancedata && assetBalancedata.balance
        ? assetBalancedata.balance
        : "0";
    ...
    const assetBalancedata2 = await this.dao.findOne(
      ...
      ["balance"]);
    const balance2 =
      assetBalancedata2 && assetBalancedata2.balance
        ? assetBalancedata2.balance
        : "0";
  ...
  async applyUnconfirmed(trs, sender) {
    const transfer = await this.getAssetObject(trs);
    const balance =
      this.balanceCache.getAssetBalance(sender.address, transfer.currency) || 0;
    ...
    this.balanceCache.setAssetBalance(
      sender.address,
      transfer.currency,
      surplus.toString()
    );
      ...
      this.balancesSequence.add(
        async cb => {
          if (
            ...
            if (
              account.multisignatures.indexOf(
  ...
  async getMyTransactions(req) {
    ...
    // 这里仅检索transfer
    const transfer = await this.queryAsset(
      where,
      null,
      true,
      pageindex,
    ...
    for (let i = 0; i < result.length; i++) {
      const newTrs = await this.runtime.transaction.serializeDbData2Transaction(
        result[i]
      );
      trslist.push(newTrs);
    }
