...
describe('AOB Test', () => {
  // 先创建两个发行商账户
  const IssuerAccount1 = Tester.randomAccount()
  const IssuerAccount2 = Tester.randomAccount()
  debug('IssuerAccount', IssuerAccount1)

  // 先给发行商想个名字
  const issuerName = Tester.randomIssuerName('', 5)
  const issuerName2 = Tester.randomIssuerName()
  ...
  beforeAll(async (done) => {
    // 转账给它
    const transaction = await createTransfer(IssuerAccount1.address, Tester.randomCoin(), Tester.Gaccount.password)
  ...
  describe('Register Issuer', () => {
    it('Should be ok', async (done) => {
      ...
      const transaction = await DdnJS.aob.createIssuer(issuerName, 'J G V', IssuerAccount1.password)
    ...
    it('with the same name again, Should be fail', async (done) => {
      ...
      const transaction = await DdnJS.aob.createIssuer(issuerName, 'J G V', IssuerAccount2.password)
    ...
    it('with the same IssuerAccount1 again, Should be fail', async (done) => {
      ...
      const transaction = await DdnJS.aob.createIssuer(issuerName2, 'J G V', IssuerAccount1.password)
    ...
    it('with IssuerAccount2 who no money, Should be fail', async (done) => {
      ...
      const transaction = await DdnJS.aob.createIssuer(issuerName2, 'J G V', IssuerAccount2.password)
      debug('注册发行商创建的transaction 3', transaction)
  ...
  describe('Get issuers', () => {
    it('Get issuers should be ok', async () => {
      const [err, res] = await Tester.apiGetAsyncE('/aob/issuers')
    ...
    it('Register issuer should be ok', async () => {
      const [err, res] = await Tester.apiGetAsyncE(`/aob/issuers/name/${issuerName}`)
      debug('issuerName', issuerName)
  ...
  describe('Register Asset', () => {
    test('Should be ok', async (done) => {
      currency = issuerName + '.' + Tester.randomIssuerName('', 3).toUpperCase()
  ...
  describe('Modify permission', () => {
    const ISSUE_ACCOUNT = Tester.genNormalAccount()
    const ISSUER_NAME = Tester.randomIssuerName()
    ...
    async function registerIssuerAsync (name, desc, { password }) {
      ...
      const res = await Tester.submitTransactionAsync(trs)
    ...
    async function registerAssetWithAllowParameters (allowWriteoff, allowWhitelist, allowBlacklist) {
      const trs = await DdnJS.aob.createAsset(ASSET_NAME, 'valid desc', MAX_AMOUNT, 1, '', allowWriteoff + '', allowWhitelist + '', allowBlacklist + '', ISSUE_ACCOUNT.password)
      const res = await Tester.submitTransactionAsync(trs)
    ...
    it('Flags modifing should be denied with special asset parameters', async (done) => {
      ...
      await Tester.giveMoneyAndWaitAsync([ISSUE_ACCOUNT.address])
      ...
      res = await Tester.apiGetAsync(`/aob/assets/${ASSET_NAME}`)
      ...
      // 改变用户权限
      res = await updateAclAsync(ASSET_NAME, '+', 0, [Tester.genNormalAccount().address].join(','), ISSUE_ACCOUNT)
      expect(res.body).to.have.property('error').to.match(/^Blacklist not allowed/)

      res = await updateAclAsync(ASSET_NAME, '+', 1, [Tester.genNormalAccount().address].join(','), ISSUE_ACCOUNT)
      expect(res.body).to.have.property('error').to.match(/^Whitelist not allowed/)
    ...
    describe('use whitelist to set acl', () => {
      const IssuerAccountWhitelist = Tester.genNormalAccount()
      const ISSUER_NAME = Tester.randomIssuerName()
      const currency = `${ISSUER_NAME}.CNY`
      const MAX_AMOUNT = '100000'

      async function registerAssetWithAllowParameters (allowWriteoff, allowWhitelist, allowBlacklist) {
        const trs = await DdnJS.aob.createAsset(currency, 'valid desc', MAX_AMOUNT, 1, '', allowWriteoff + '', allowWhitelist + '', allowBlacklist + '', IssuerAccountWhitelist.password)
        const res = await Tester.submitTransactionAsync(trs)
      ...
      it('Valid allow parameters', async () => {
        await Tester.giveMoneyAndWaitAsync([IssuerAccountWhitelist.address])
      ...
      it('Transfer outside whitelist firstly, Should be fail', async (done) => {
        ...
        const obj = {
          recipientId: Tester.Daccount.address,
          currency,
          aobAmount: '10',
          message: '测试转账失败'
        }
      ...
      it(`Add ${Tester.Daccount.address} to whitelist, Should be ok`, async (done) => {
        const obj = {
          currency,
          flag: 1,
          operator: '+',
