...
export const Transfer = () => {
  describe('dapp transfer', () => {
    ...
    it(`should be ok to transfer ${DAPP_CURRENCY} to an app`, async done => {
      ...
      const balance = DdnUtils.bignum.minus(balance1, inTransferAmount, node.constants.fees.transfer)
    ...
    it(`should be ok to transfer ${DAPP_CURRENCY} from app to account`, async done => {
      ...
      expect(DdnUtils.bignum.minus(dappBalance1, amount, node.constants.fees.transfer).toString()).to.equal(
        dappBalance2
      )
