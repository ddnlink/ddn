	...
	describe("crypto.js", () => {
		...
		describe("#getFee", () => {
			...
			it("should return number", () => {
				const fee = getFee({ amount: "100000", type: 0 });    //Bignum update
                
                //Bignum update (fee).to.be.a("number");
			...
			it("should return 10000000", () => {
				const fee = getFee({ amount: "100000", type: 0 });    //Bignum update
                //Bignum update (fee).to.be.a("number").to.equal(10000000);
                expect(fee).to.be.a("string").to.equal("10000000");
			});

			it("should return 10000000000", () => {
				const fee = getFee({ type: 1 });
                //Bignum update (fee).to.be.a("number").to.equal(10000000000);
                expect(fee).to.be.a("string").to.equal("10000000000");
			});

			it("should be equal 1000000000000", () => {
				const fee = getFee({ type: 2 });
                //Bignum update (fee).to.be.a("number").to.equal(1000000000000);
                expect(fee).to.be.a("string").to.equal("1000000000000");
			});

			it("should be equal 100000000", () => {
				const fee = getFee({ type: 3 });
                //Bignum update (fee).to.be.a("number").to.equal(100000000);
                expect(fee).to.be.a("string").to.equal("100000000");
			});
		});
		...
		describe("#getKeys", () => {
			...
			it("should return two keys in hex", () => {
				...
				expect(keys.publicKey).to.be.a("string").and.match(() => {
					try {
				...
				(keys.privateKey).to.be.a("string").and.match(() => {
					try {
		...
		describe("#generateAddress", () => {
			...
			it("should generate address by publicKey", () => {
				...
				expect(address).to.equal("AFkctfgZFkaATGRhGbj72wzJqACvMyzQ1U");
			});
		});
