...
class AssetBase {
  ...
  async calculateFee() {
    return DdnUtils.bignum.multiply(
      this.constants.net.fees.transfer,
  ...
  async hasExtProps() {
    if (
      this.isHasExtProps !== null &&
      typeof this.isHasExtProps !== "undefined"
    ) {
    ...
    mapping.forEach(item => {
      if (item) {
        if (
          item.field &&
          item.field.substring(item.field.length - 4, item.field.length) ===
            "_ext"
        ) {
          ...
          if (orderItem.length === 2) {
            if (
              ...
              if (fieldName) {
                ...
                this.logger.warn(
                  `Invalid order field: ${JSON.stringify(orderItem)}`
                );
              }
            } else {
            ...
            this.logger.warn(
              `Invalid order item: ${JSON.stringify(orderItem)}`
            );
          }
        } else {
      ...
      if (item) {
        ...
        if (fieldType === "str") {
          ...
          if (intValue !== null && typeof intValue !== "undefined") {
            ...
            if (itemRule) {
              if (
                itemRule.maxValue !== null &&
                typeof itemRule.maxValue !== "undefined"
              ) {
              ...
              if (
                itemRule.minValue !== null &&
                typeof itemRule.minValue !== "undefined"
              ) {
          ...
          if (
            ...
            if (
              ...
              typeof timestampValue.getDate !== "function"
            ) {
              try {
                // FIXME: 你要干嘛？
  ...
  async verify(trs) {
    if (DdnUtils.bignum.isZero(trs.amount)) {
      ...
      if (trs.recipientId) {
        // wxm block database
        throw new Error(
          "The recipientId attribute of the transaction must be null."
        );
      }
      ...
      if (!trs.recipientId) {
        // wxm block database
        throw new Error(
          "The recipientId attribute of the transaction can not be null."
        );
      }
    }
  ...
  async objectNormalize(trs) {
    ...
    if (validateErrors) {
      this.logger.error(
        `Can't parse asset ${assetName}: ${validateErrors[0].schemaPath} ${validateErrors[0].message}`
      );
      throw new Error(
        `Can't parse asset data: ${validateErrors[0].schemaPath} ${validateErrors[0].message}`
      );
    }
