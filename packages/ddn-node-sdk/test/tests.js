...
describe("DDN JS", function () {
	...
	it("should have properties", function () {
		...
		properties.forEach(function (property) {
			(ddn).should.have.property(property);
		});
	});

	describe("crypto.js", function () {
		var crypto = ddn.crypto;
	...
	describe("dapp.js", function () {
		...
		describe("#createDApp", function () {
			...
			describe("returned dapp", function () {
				var keys = ddn.crypto.getKeys("secret");
				var secondKeys = ddn.crypto.getKeys("secret 2");
				...
				it("should be ok to verify bytes", function () {
					...
					var secret = 'secret1'
					var keys = ddn.crypto.getKeys(secret)
					var signature = ddn.crypto.signBytes(data1, keys)
	...
	describe("transaction.js", function () {
		var transaction = ddn.transaction;
		...
		describe("#createTransaction with second secret", function () {
			...
			var keys = ddn.crypto.getKeys(secondSecret);
	...
	describe("vote.js", function () {
		...
		describe("#createVote", function () {
			var createVote = vote.createVote,
				vt = null,
				publicKey = ddn.crypto.getKeys("secret").publicKey,
